        ...
        <div className="w-full max-w-5xl mx-auto mb-12">
            <div className="flex items-center">
                {STAGES.map((stage, index) => {
                    ...
                    // A stage is clickable if it's been completed.
                    // Continuity stage is special: it's clickable as soon as the 'director' stage is reached or passed.
                    ...
                    return (
                        <React.Fragment key={stage.id}>
                             <button
                                ...
                                className={`flex flex-col items-center text-center w-32 group transition-transform duration-200 ${isClickable ? 'cursor-pointer hover:scale-105' : 'disabled:cursor-not-allowed'}`}
                            >
                                <div className={`flex items-center justify-center w-12 h-12 rounded-full border-2 transition-all duration-500 ${
                                    ...
                                    {isCompleted && !isCurrent ? <CheckCircleIcon className="w-6 h-6 text-white" /> : stage.icon}
                                </div>
