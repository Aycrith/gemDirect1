...
const StoryIdeaForm: React.FC<StoryIdeaFormProps> = ({ onSubmit, isLoading }) => {
    ...
    const quickStarts = [
        "A detective in a cyberpunk city discovers a conspiracy that goes to the very top.",
        "Two rival magicians in the 19th century compete to create the ultimate illusion.",
        "A lone astronaut stranded on a distant planet must survive with limited resources.",
    ];
    ...
    return (
        <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="mt-8 text-left">
                <p className="text-sm text-gray-500 mb-3">Or try a quick start:</p>
                <div className="space-y-2">
                    {quickStarts.map((qs, i) => (
                         <button key={i} onClick={() => setIdea(qs)} className="text-left text-sm text-indigo-400 hover:text-indigo-300 bg-gray-800/50 p-2 rounded-md w-full transition-colors">
                            {qs}
                         </button>
                    ))}
                </div>
