...
This plan maps the next-phase implementation work needed to connect the gemDirect1 story generator to the ComfyUI WAN-first workflows so the entire e2e helper can run unattended. Canonical: WAN T2I for keyframes (image_netayume_lumina_t2i.json) and WAN 2.2 5B ti2v for video (ideo_wan2_2_5B_ti2v.json). SVD remains optional/regression only. The scope aligns with the user request (Sections A-F).
...
- Guardrails now prepend `SINGLE_FRAME_PROMPT` to every still prompt and append the shared `NEGATIVE_GUIDANCE` clause to each negative prompt via `extendNegativePrompt()` so WAN outputs center on one cinematic moment per shot. `services/__tests__/comfyGuardrails.test.ts` asserts these strings, and the helper logs them per shot for Playwright/QA traceability.
- WAN 5B automation lives in `scripts/generate-scene-videos-wan2.ps1` (HttpClient upload + first `LoadImage` wiring) followed by `scripts/update-scene-video-metadata.ps1` (forward-slashed MP4 paths). These scripts are Step 11b/11c inside `scripts/run-comfyui-e2e.ps1` so every mp4 + metadata entry is recorded after `comfyui-status` has confirmed the canonical workflows and key mappings.
- `scripts/run-comfyui-e2e.ps1` now runs `scripts/preflight-mappings.ts` as Step 0 and persists both helper summaries under `HelperSummaries.MappingPreflight` / `HelperSummaries.ComfyUIStatus` inside `artifact-metadata.json`/`public/artifacts/latest-run.json`, so the Artifact Snapshot, Timeline, and Playwright attachments can all link to the same telemetry JSONs.
...
-5. **Mapping preflight & WAN video automation**: Step 0 requires `scripts/preflight-mappings.ts --project ./exported-project.json --summary-dir test-results/comfyui-status` so the helper confirms the wan-i2v CLIP/LoadImage wiring, auto-infers inline mappings, writes `mapping-preflight.json` plus a `unit` mirror, and exits 3 when the contract is still missing. Every helper output (summary JSON and verbose log) lives under `test-results/comfyui-status/` and is mirrored via `HelperSummaries.MappingPreflight` / `HelperSummaries.ComfyUIStatus` in `artifact-metadata.json` and `public/artifacts/latest-run.json` so UI cards, Playwright helpers, and the validator reference the same telemetry. For WAN video renders, `scripts/generate-scene-videos-wan2.ps1` uploads the first keyframe, injects it into the `wan-i2v` workflow, enforces mp4/libx264 with a deterministic `logs/<ts>/video/<sceneId>/<sceneId>.mp4` prefix, and logs the `DurationSeconds`/`PollIntervalSeconds`/`HistoryExitReason` lines that `scripts/validate-run-summary.ps1` later checks. `scripts/update-scene-video-metadata.ps1` then scans those MP4s, normalizes forward slashes, records `Path`, optional `DurationSeconds` (via `ffprobe` when available), `Status`, `UpdatedAt`, and `Error`, and keeps the validator/UI/Playwright telemetry contract satisfied.
-6. **Docs-first guardrail**: Before touching scripts, validators, or UI, read the required meta docs (README.md, DOCUMENTATION_INDEX_20251111.md’s “Required Telemetry & Queue Policy Orientation,” STORY_TO_VIDEO_TEST_CHECKLIST.md, WORKFLOW_FIX_GUIDE.md, HANDOFF_SESSION_NOTES.md, QUICK_START_E2E_TODAY.md, REFERENCE_CARD_QUICK.md, WINDOWS_AGENT_TEST_ITERATION_PLAN.md, and notes/codex-agent-notes-20251111.md) so the LM Studio health check, queue knobs, telemetry enforcement, and artifact expectations are top of mind.
-7. **UI metadata handshake**: Artifact Snapshot/Timeline panels mirror logs/<ts>/artifact-metadata.json and public/artifacts/latest-run.json, so queue policy cards, telemetry badges, GPU stats, fallback warnings, archive links, and LLM metadata in the UI must always match what the helper logs and what the validator enforces.
