...
test.describe('Image Synchronization (ComfyUI → React)', () => {
  ...
  test.skip('validates keyframe images sync to UI after generation', async ({ page }) => {
    ...
    console.log(`🎬 Starting generation of ${scenesNeedingImages} keyframes...`);
    
    await generateButton.click();

    // Wait for generation to complete (3 scenes × ~30s each = ~90s max)
    ...
    for (let i = 0; i < mockScenes.length; i++) {
      ...
      try {
        ...
        console.log('📊 IndexedDB generatedImages:', JSON.stringify(dbImages, null, 2));
    ...
    console.log(`📊 Final IndexedDB state: ${JSON.stringify(Object.keys(finalImagesState || {}))}`);
    ...
    mockScenes.forEach(scene => {
      ...
      console.log(`✅ Scene "${scene.title}" persisted to IndexedDB`);
    });

    // Validate sync logs
    expect(syncLogs.length).toBeGreaterThan(0);
    console.log(`📋 Captured ${syncLogs.length} sync log messages`);
    
    // Each scene should have a success log
    mockScenes.forEach(scene => {
      const sceneLog = syncLogs.find(log => log.includes(scene.id) && log.includes('✅'));
  ...
  test.skip('validates images persist after page reload', async ({ page }) => {
    ...
    console.log('✅ All images visible before reload');
    ...
    for (const scene of mockScenes) {
      ...
      const imgSrc = await sceneImage.getAttribute('src');
      expect(imgSrc).toContain('data:image');
      console.log(`✅ Scene "${scene.title}" persisted after reload`);
    }

    console.log('✅ All images persisted correctly after page reload');
  });
  ...
  test('validates generatedImages state structure', async ({ page }) => {
    ...
    console.log('✅ IndexedDB has "misc" store for generatedImages');
    ...
    console.log('✅ generatedImages state structure is valid');
  });
});
